...
function init() {
  ...
  sceneMain.background = new THREE.Color( 'darkblue' );
  ...
  var lineMaterial = new THREE.LineBasicMaterial({ color: 'lightgrey' });
  for (x = -0.5; x <= 0.5; x++) {
    ...
    for (i = 0; i <= gameHeight * 1.5; i++) {
      ...
      sceneMain.add(line);
      if (i == gameHeight) {
        lineMaterial = new THREE.LineBasicMaterial({ color: 'lightgrey' });
      }
    }
  }
